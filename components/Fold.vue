<template>
  <article class="my-48 mt-80 font-sans">
    <p
      class="text-2xl font-medium text-gray-800 dark:text-gray-200 lg:text-3xl"
    >
      Hey there,
    </p>
    <p class="mb-3 text-6xl font-medium">
      I'm <span class="font-mono text-teal-400">Jarne Delarue</span>
    </p>
    <p class="pb-6 text-3xl text-neutral-400">
      A full stack web developer and linux enthousiast
    </p>
    <nuxt-link
      to="/#about"
      class="rounded-lg border-2 border-solid border-teal-400 p-2 outline-none hover:bg-gray-800 focus:bg-gray-700 focus-visible:ring focus-visible:ring-teal-600"
    >
      Find out more
    </nuxt-link>
  </article>
</template>
